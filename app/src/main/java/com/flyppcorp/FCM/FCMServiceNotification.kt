...
class FCMServiceNotification : FirebaseMessagingService() {
    ...
    private fun descontoNotification(payload: MutableMap<String, String>) {
        ...
        builder.setAutoCancel(true)
            .setSmallIcon(R.drawable.ic_notification_new_logo)
            .setStyle(NotificationCompat.BigTextStyle()
                .setBigContentTitle(payload.get("titleKeyMain"))
                .bigText(payload.get("bodyKeyMain")))
            .setSound(alarm)
            .setContentTitle(payload.get("titleKeyMain"))
            .setContentText(payload.get("bodyKeyMain"))
            .setContentIntent(pItent)

        val random = java.util.Random().nextInt(500)

        if (payload.get("senderKeyMain") == null){
            return
        }else{
            notificationManager1.notify(random, builder.build())
        }
